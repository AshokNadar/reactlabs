...
@Component({
  selector: 'app-testing-standrad',
  templateUrl: './testing-standrad.component.html',
  styleUrls: ['./testing-standrad.component.css']
})
export class TestingStandradComponent implements OnInit {
  ...
  standardList(){
    ...
    .subscribe(
      data =>{
        this.loading = false;
        (data!=null)?this.listStandard = data:this.listStandard = [];
      },error =>{
        this.loading = false;
        if(error.length>0){
  ...
  create(){
    ...
    this.loading = true;
    this.masterService.createTestingStandard(this.addForm.value)
    .subscribe(
      data =>{
        this.loading = false;
  ...
  editData(id){
    this.listStandard.forEach(list =>{
      if(id == list.id){
  ...
  update(){
    ...
    this.loading = true;
    this.masterService.updateTestingStandard(this.editForm.value)
    .subscribe(
      data =>{
        this.submitted = false;
        this.loading = false;
  ...
  deleteItem(id){
    this.masterService.deleteTestingStandard(id)
    .subscribe(
      data =>{
